...
def build_is_not_none_expr() -> BuilderCase:
    with build_module("types") as mod:
        ...
        return BuilderCase(
            builder=mod,
            expected_code="maybe = body if test is not None else None",
        )
...
def build_list_const() -> BuilderCase:
    with build_module("types") as mod:
        ...
        return BuilderCase(
            builder=mod,
            expected_code="result = [1, 2, foo, bar]",
        )
...
def build_list_compr_expr() -> BuilderCase:
    with build_module("types") as mod:
        mod.assign_stmt(
            "ys",
            mod.list_expr(mod.compr(mod.attr("x"), mod.attr("xs")), mod.attr("x") * mod.const(2)),
        )

        return BuilderCase(
            builder=mod,
            expected_code="ys = [x * 2 for x in xs]",
        )
...
def build_node_t_module() -> ModuleASTBuilder:
    with build_module("node_t") as mod:
...
def build_type_alias_module() -> ModuleASTBuilder:
